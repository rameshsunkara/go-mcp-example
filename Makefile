...
TEST_COVERAGE_THRESHOLD := 40 # change to 80 later
...
## Start the MCP server via stdio (default transport)
.PHONY: start
start: run ## Start the MCP server via stdio (default transport)

## Run the MCP server via stdio
.PHONY: run
run: ## Run the MCP server via stdio
	go run $(LDFLAGS) main.go

## Run the MCP server via HTTP (for debugging)
.PHONY: run-http
run-http: ## Run the MCP server via HTTP (for debugging)
	go run $(LDFLAGS) main.go --http localhost:8080
...
.PHONY: version
version: ## Display the current version of the MCP server
	@echo $(VERSION)
...
## Run all CI checks locally (format, lint, build, test with coverage)
.PHONY: ci-local
ci-local: ## Run all CI checks locally (format, lint, build, test with coverage)
...
owasp-report: ## Generate OWASP report
	vacuum html-report -z openapi.yaml
